...
function LoadShop()
	...
	local shopResult = MySQL.Sync.fetchAll('SELECT * FROM naroutil')
...
ESX.RegisterServerCallback('esx_naroutil:requestDBItems', function(source, cb)
	if not hasSqlRun then
...
RegisterServerEvent('esx_naroutil:buyItem')
AddEventHandler('esx_naroutil:buyItem', function(itemName, amount, zone)
	...
	if amount < 0 then
		print('esx_naroutil: ' .. xPlayer.identifier .. ' attempted to exploit the shop!')
		return
	end
...
ESX.RegisterUsableItem('blowpipe', function(source)
  ...
  TriggerClientEvent('esx_naroutil:onHijack', _source)
  TriggerClientEvent('esx:showNotification', _source, _U('you_used_blowtorch'))

end)

RegisterServerEvent('esx_naroutil:armorremove')
AddEventHandler('esx_naroutil:armorremove', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeInventoryItem('armor', 1)
end)

ESX.RegisterUsableItem('armor', function(source)
	TriggerClientEvent('esx_naroutil:armor', source)
end)

RegisterServerEvent('esx_naroutil:handcuffremove')
AddEventHandler('esx_naroutil:handcuffremove', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeInventoryItem('handcuff', 1)
end)

RegisterServerEvent('esx_naroutil:handcuff')
AddEventHandler('esx_naroutil:handcuff', function(target)
  TriggerClientEvent('esx_naroutil:handcuff', target)
end)
...
AddEventHandler('esx_naroutil:cutting_pliers', function(target)
  TriggerClientEvent('esx_naroutil:cutting_pliers', target)
end)
